...
min_date = datetime.fromtimestamp(initial_df.min()["Unix_Time"]).replace(hour=0, minute=0, second=0, microsecond=0)
max_date = datetime.fromtimestamp(initial_df.max()["Unix_Time"]).replace(hour=0, minute=0, second=0, microsecond=0)
...
app.layout = html.Div([
  html.Div([
    ...
    html.Div([
      html.Div([
        html.P(children = constant.outlier_detection_text[True],
               id = "outlier-detection-label"),
        daq.BooleanSwitch(on = True,
                          color = "#72b075",
                          id = "outlier-detection-switch",
                          style = {"display": "inline-block"})
      ])        
    ], style={"width": "15%", "margin-left": "10pt", "padding": "5pt"}),
...
@app.callback(
    ...
    Input(component_id = "outlier-detection-switch", component_property = "on"),
...
def update_plot(observation_start_date,
  ...
  group_logs_status = "grouped" if "grouped" in check_list else "not_grouped"
...
@app.callback(
    Output(component_id = "outlier-detection-label", component_property = "children"),
...
def update_options(pause_info,
  ...
  outlier_detection_label = constant.outlier_detection_text[outlier_detection_switch]
  ...
  return outlier_detection_label, general_time_off_task, general_time_off_task_suggestion, component_time_off_task, component_options, None, active_component_thresholds, active_component_thresholds
...
@app.callback(
    ...
    Input(component_id = "pause-info", component_property = "data"),
    State(component_id = "type-of-session-toggle", component_property = "value"),
    State(component_id = "outlier-detection-switch", component_property = "on"), prevent_initial_call=True)
