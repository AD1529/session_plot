...
def transform_active_components_options_to_dict(options_list):
  ...
  for option in options_list:
    temp = option.split(" (")
    res[temp[0]] = int(temp[1].split(" min)")[0])
  return res
...
def get_classified_pause_length_list(pause_analysis, type_of_session, component_toggle):
  ...
  if len(result_list[0]) == len(result_list[1]):
    result_list = [result_list[0], np.append(0, result_list[1])]
  return np.array(result_list, dtype=object) 

def normalize_bins(n):
  for i in range(len(n[0])):
    total_num_in_bin = 0
    for j in range(len(n)):
      total_num_in_bin += n[j][i]
    for j in range(len(n)):
      n[j][i] = round(n[j][i]*100.0/total_num_in_bin, 2)
  return n

def get_grouped_tipical_behavior(data, keys, start_point, end_point, step):
  bins = list(range(start_point, end_point, step))
  bins.append(end_point)
  n, bins, patches = plt.hist(data, bins, histtype="bar")
  plt.close()
  ...
  for i in range(len(n[0])):
    ...
    for j in range(len(n)):
      l.append(n[j][i])
    tipical_behavior.append(l.index(max(l)))
  grouped_tipical_behavior = []
  current_value = tipical_behavior[0]
  start_index = 0 
  for i in range(len(tipical_behavior)):
    if tipical_behavior[i] != current_value:
      grouped_tipical_behavior.append((start_point + start_index*step, start_point + i*step, keys[current_value]))
      start_index = i
      current_value = tipical_behavior[i]
  grouped_tipical_behavior.append((start_point + start_index*step, end_point, keys[current_value]))
  return grouped_tipical_behavior
...
def get_recommended_threshold(data, labels):
  ...
  while start_point != end_point:
    grouped_tipical_behavior = get_grouped_tipical_behavior(data, labels, start_point, end_point, step)
    start_point = grouped_tipical_behavior[0][0] if grouped_tipical_behavior[0][2] in constant.stt_suggestion_real_pause else grouped_tipical_behavior[0][1]
    end_point = grouped_tipical_behavior[-1][0] if grouped_tipical_behavior[-1][2] in constant.stt_suggestion_real_pause else grouped_tipical_behavior[-1][1]
    if step == 1:
      fluctuation_info = (start_point, end_point)
    step += 1
  return fluctuation_info, start_point
...
def get_general_time_off_task_update(final_pause_analysis, type_of_session, moodle_course, study_session_identification, outlier_detection_switch):
  ...
  if "stt" in study_session_identification:
    ...
    if outlier_detection_switch:
      ...
      fluctuation_info, time_off_task_suggestion = get_recommended_threshold(preprocessed_analysis, constant.stt_suggestion_final_pause_types[type_of_session])
      general_time_off_task_suggestion = constant.suggestion_threshold.format(fluctuation_info[0], fluctuation_info[1], time_off_task_suggestion)
...
def get_component_time_off_task_suggestion(final_pause_analysis, type_of_session, outlier_detection_switch, component_toggle):
  ...
  if outlier_detection_switch and component_toggle is not None: 
    ...
    if len(preprocessed_analysis[0]) != 0 or len(preprocessed_analysis[1]) != 0:
      fluctuation_info, time_off_task_suggestion = get_recommended_threshold(preprocessed_analysis, constant.stt_suggestion_final_pause_types[type_of_session])
      component_time_off_task_suggestion = constant.suggestion_threshold.format(fluctuation_info[0], fluctuation_info[1], time_off_task_suggestion)
    else:
      component_time_off_task_suggestion = constant.no_breaks_caused_text
  return component_time_off_task_suggestion

def unix_to_normal(unix_time):
...
def get_data_for_boxplot(learning_sessions):
  ...
  for session_times in learning_sessions:
    ...
    start_session = unix_to_normal(session_times[0])
    ends_unix.append(int(session_times[1]))
    end_session = unix_to_normal(session_times[1])
